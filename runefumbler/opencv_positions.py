...
def build_ge_offer():
    ...
    assert (
        template_set_price is not None
    ), "file could not be read, check with os.path.exists()"

    template_confirm = cv.imread("photos/confirm.png", cv.IMREAD_GRAYSCALE)
    assert (
        template_confirm is not None
    ), "file could not be read, check with os.path.exists()"

    template_quantity = cv.imread("photos/quantity.png", cv.IMREAD_GRAYSCALE)
    assert (
        template_quantity is not None
    ), "file could not be read, check with os.path.exists()"

    match_template_update_all_set_price(
        img_rgb, img_gray, template_set_price, (0, 0, 255)
    )
    match_template_update_all_confirm(
        img_rgb, img_gray, template_confirm, (0, 255, 0)
    )
    match_template_update_all_quantity(
        img_rgb, img_gray, template_quantity, (255, 0, 0)
    )
