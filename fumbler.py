...
def position_to_click(index):
    x = positions[index][0]
    y = positions[index][1]
    mouse.moveTo(random.randint(x - 10, x + 10), random.randint(y - 10, y + 10), random.uniform(.1, 1), mouse.easeInOutSine)
    mouse.click()
...
def execute_trades():
    while True:
        ...
        if len(user_input) == 2 and user_input[0].isdigit() and int(user_input[0]) in range(1, 9):
